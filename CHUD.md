...
python safety/CHUD_LoX.py \
    ...
    --limit-memory
```
> By default this loads the `Llama-2-7b-chat-hf` and `Llama-2-7b-hf` models from Huggingface. May require you to authenticate. If using local models, use the `--model` and `--base-model` flags. While both options use the same amount of memory, note that streaming the models vs downloading and using from local took my NVIDIA 4080 Laptop GPU about 15 mins vs like 5 mins.
